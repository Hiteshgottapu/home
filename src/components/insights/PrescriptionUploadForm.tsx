...
export function PrescriptionUploadForm({ onUploadSuccess }: PrescriptionUploadFormProps) {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      setAnalysisResult(null); // Reset previous result on new file selection
    } else {
      setFileName(null);
    }
  };
  ...
  const onSubmit: SubmitHandler<PrescriptionUploadFormValues> = async (data) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        try {
          const result = await extractMedicationDetails({ prescriptionDataUri: base64data });
          setAnalysisResult(result);
          const newPrescription: Prescription = {
            id: `pres_${Date.now()}`,
            fileName: file.name,
            uploadDate: new Date().toISOString(),
            status: 'needs_correction', // Default to needs_correction after AI analysis
            extractedMedications: result.medicationDetails,
          ...
          toast({
            ...
            description: "Could not analyze the prescription. Please try a clearer image or different file.",
            variant: "destructive",
          });
        } finally {
          setIsLoading(false);
      ...
      reader.onerror = () => {
        ...
        toast({ title: "File Reading Error", description: "Could not read the file.", variant: "destructive" });
        setIsLoading(false);
      };
    } catch (error) {
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-center w-full">
                <label htmlFor="prescriptionFile" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer bg-muted/50 hover:bg-muted/80 border-border hover:border-primary transition-colors">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        {fileName && <p className="text-xs text-accent mt-2"><FileText className="inline h-3 w-3 mr-1" />{fileName}</p>}
                    </div>
                    <Input 
                      id="prescriptionFile" 
                      type="file" 
            ...
            {errors.prescriptionFile && <p className="text-sm text-destructive mt-1">{errors.prescriptionFile.message}</p>}
          </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            <Card className="bg-background/50">
              <CardContent className="p-4 space-y-3">
                ...
                {analysisResult.ocrConfidence <= 0.7 && <p className="text-xs text-amber-700">Low confidence: please verify details carefully.</p>}
                ...
                {analysisResult.medicationDetails.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground">No medication details extracted. Try a clearer image.</p>
                )}
                 <p className="text-xs text-muted-foreground pt-2">Please verify these details against your prescription and make corrections if needed in the list below.</p>
              </CardContent>
            </Card>
          </div>
        )}
